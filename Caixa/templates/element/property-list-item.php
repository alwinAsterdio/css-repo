...
$target = !empty($website_url) ? ' target="_blank" rel="noopener"' : '';
...
<a href='<?= $property_url ?>'
    ...
    class="property-item property-list-item <?= !empty($website_url) ? ' property-item--website-url' : '' ?>"
    ...
    <div class="property-list-item__featured_photo">
        <img src="<?= $property->getFeaturedPhotoURL('medium') ?>" alt="property-image" loading="lazy" class="property-list-item__featured_photo__img h-full" onerror="featuredPhotoOnError(this)"/>
    </div>
    <div class="property-list-item__details">
        <div class="property-list-item__details__name">
            <h2 class="property-list-item__details__name__content">
                <?= $property->get('name') ?>
            </h2>
            <?php if (!empty($agent_logo)) { ?>
                <div class="property-list-item__details__name__agent">
                    <img src="<?= $agent_logo ?>" alt="agent-logo" loading="lazy" class="property-list-item__details__name__agent__logo" onerror="agentPhotoOnError(this)"/>
                </div>
            <?php } ?>
        </div>
        <h3 class="property-list-item__details__price">
            ...
            if (!empty($price_diff) && $price_diff < 0) {
                ...
                <span class="price-diff price-diff--down">
                    ...
                    Ha bajado <span><?= \App\Utils\CommonFunctions::getDataFormat(abs($price_diff), '', ['unit' => 'currency'], true) ?></span>
                </span>
                <?php
            }
            ?>
        </h3>
        <div class="property-list-item__details__features">
            <?= $this->element('features_short', ['property' => $property]); ?>
        </div>
        <div class="property-list-item__details__description">
            <div>
                <?= \App\Utils\CommonFunctions::trimWords($property->getField('description'), 50) ?>
            </div>
        </div>
        <div class="property-list-item__details__buttons">
            <button class="contact-us-btn contact-us-btn"><?= $page_data_options['feature_properties_contact_btn'] ?></button>
