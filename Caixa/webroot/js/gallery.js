const CaixaGalleryObject = () => {
    ...
    /**
     * Fetches media data for a property and invokes a callback to render the gallery.
     *
     * @param {HTMLElement} gallery_elm The gallery element where the slider will be rendered.
     * @param {function} callback_method A callback function to handle the rendering of the gallery.
     *                                   It receives two arguments:
     *                                   - {HTMLElement} gallery_elm: The gallery element.
     *                                   - {Array<string>} photos_urls: An array of photo URLs.
     */
    obj.render_slider_gallery = function(gallery_elm, callback_method) {
        const property_elem = gallery_elm.closest('.property-item')
        if (property_elem === null) {
            return;
        }

        const property_id = property_elem.getAttribute('data-ref') ?? property_elem.getAttribute('id')

        fetch('/api/property-public/media/' + property_id, {
            method: 'POST',
            ...
            callback_method(gallery_elm, photos_urls)
        })
    }
    ...
    /**
     * Render the slider gallery html for single property page.
     *
     * @param {object} elem The element.
     * @param {array} photos_urls The photo urls.
     */
    obj.render_slider_gallery_single_page_html = function(elem, photos_urls) {
        const gallery_side_elem = elem.parentNode.querySelector('.gallery-side');
        if (photos_urls.length > 2) {
            elem.classList.remove('full-width')
            const second_photo_elem = elem.parentNode.querySelector('.gallery-second-photo');
            second_photo_elem.setAttribute('onerror', 'featuredPhotoOnError(this)')
            second_photo_elem.src = photos_urls[1]

            const third_photo_elem = elem.parentNode.querySelector('.gallery-third-photo');
            third_photo_elem.setAttribute('onerror', 'featuredPhotoOnError(this)')
            third_photo_elem.src = photos_urls[2]

            gallery_side_elem.classList.remove('gallery-side--loading')
        } else {
            gallery_side_elem.remove();
            elem.classList.add('full-width')
        }

        if (photos_urls.length) {
            CaixaGallery.render_slider_gallery_html(elem, photos_urls)
        }
    }
    ...
    obj.enable_gallery = function(){
        ...
        gallery_sections.forEach(gallery_elm => {
            ...
            obj.render_slider_gallery(gallery_elm, obj.render_slider_gallery_html)
        })

        const single_page_gallery_sections = document.querySelectorAll('.qoetix-gallery-section--single-auto:not(.has-website-url)');
        single_page_gallery_sections.forEach(gallery_elm => {
            gallery_elm.id = 'gallery-' + index++;
            obj.render_slider_gallery(gallery_elm, obj.render_slider_gallery_single_page_html)
        })
    }

    obj.add_buttons = function (){
        const gallery_actions = document.querySelectorAll('.gallery-actions')
        gallery_actions.forEach(gallery_elm => {
            gallery_elm.insertAdjacentHTML("beforeend", `<div class="gallery-pagination hidden gallery-actions_btn">
                ${video_btn}
            </div>`)
        })
    }
    ...
    obj.render_gallery_popup = function(btn_elem) {
        ...
        const contact_btn_lbl = btn_elem.getAttribute('data-contact-btn-label')
        const all_images = parent.querySelectorAll('.glide__slide:not(.glide__slide--clone) .gallery-img')
        const agent_email = btn_elem.getAttribute('data-agent-email')
        ...
        let popup_html = `<div class="gallery-popup">
            <div class="gallery-popup__row gallery-popup__row--header">
                ...
                <div class="gallery-popup__row__col">
                    <div class="gallery-popup__row__col__row">
                        ${agent_email !== '' ? '<button class="btn btn-primary btn-pill open-contact-form-btn">' + contact_btn_lbl + '</button>' : ''}
                    </div>
                </div>
            </div>
...
const CaixaGallery = CaixaGalleryObject();

(function () {
    document.addEventListener('DOMContentLoaded', function (event) {
        CaixaGallery.enable_gallery()
        ...
        show_gallery_btns.forEach(elem => {
            elem.addEventListener('click', function(){
                CaixaGallery.render_gallery_popup(elem)
            })
        });
    })
})()
