...
<?= $this->Html->css('/vip_portal/css/vip_admin/tw-properties'); ?>
...
<div class="va_content_wrapper py-5 px-6 md:px-10">
    ...
    <div class="search-menu pb-5 md:pb-0 my-5">
        <?= \App\Utils\Search::render('vipPortal', ['row_class' => 'grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5', 'mobile_filter' => true, 'action' => \App\Utils\SitesPage::getFirstPageUrlByTemplate('vip_admin/properties')]); ?>
    </div>
    <?php
    $page = 1;
    ...
    $property_data = \App\Connector\Property::searchProperties($query_params, $page, 20);
    ...
    <div class='properties-listing-section mt-5'>
        <?php if (!empty($property_data['data'])) : ?>
            <div
                class="inline-labels flex flex-wrap justify-end md:justify-between m-mobile:flex-row md:items-center gap-5 sort_container">
                <?php
                $field_details = [
                    'id' => 'sort',
                    'name' => 'sort',
                ...
                ?>
                <div class="hidden md:block w-64 select_placeholder_container">
    ...
    <?php if (!empty($property_data['pagination']['pageCount']) && $property_data['pagination']['pageCount'] > 1) : ?>
        <div
...
<script>
    ...
    function addClassToCircleIndicator() {
        document.getElementById("circle_indicator_all").classList.add("circle_indicator_active");
    }
