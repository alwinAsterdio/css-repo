...
<?= $this->Html->css('/ProjectPro/css/tw-property'); ?>
...
<div class="box-border relative w-full p-[30px] sm:p-10 md:p-16 2xl:p-20 overflow-hidden property-intro">
    ...
    <button class="mt-6 btn-bg-theme-color" onClick="document.getElementById('contact-us').scrollIntoView();">
...
<!-- Property Essentials -->
<div class="p-[30px] sm:p-10 md:p-16 xl:p-0 xl:px-20 xl:pt-20 property-features">
    ...
    <div class="flex flex-wrap gap-[15px] sm:gap-5 xl:gap-[30px] xl:w-11/12 mt-6 xl:mt-9">
        <?php $features_list = \App\Utils\Config::getList('features', $property->get('property_type')); ?>
        <?php if (!empty($features_list)) : ?>
            <?php foreach ($features_list as $field_name) : ?>
                <?php
                $value = $property->getField($field_name);
                if (empty($value)) {
                    continue;
                }
                ?>
                <div class="px-3.5 2xl:px-6 py-3.5 2xl:py-4 rounded-xl border-solid border border-[#DCEFF2] bg-[#ECF7F9]">
                    <div class="text-xs xl:text-sm 2xl:text-base text-theme-primary-color">
                        <?= $property->getLabel($field_name) ?>
                    </div>
                    <div class="text-sm capitalize xl:text-base 2xl:text-xl">
                        <?= $value ?>
                    </div>
                </div>
            <?php endforeach; ?>
...
<div class="xl:m-20 bg-[#1C2526] xl:relative xl:h-[45rem] box-border xl:rounded-xl contact-us">
    ...
    <div class="box-border z-20 px-6 py-16 text-center xl:absolute xl:text-left xl:py-40 xl:w-1/2 xl:pl-36 form" >
        ...
        <button id="submit-btn" class="mt-5 btn-bg-theme-color g-recaptcha" type="submit" onClick="javascript:submit_lead_form({obj: this, form_id: 'enquiry-form'})">
