...
<div class="bg-theme-secondary-color text-theme-tertiary-color pt-14 pb-5">
    <div class="max-w-[1400px] mx-auto px-5">
        <div class="flex flex-col gap-2 md:flex-row justify-around text-center md:text-left">
            <div class="flex flex-col items-center mb-5">
                <a href="/" class="no-underline">
                    <img src="<?= \App\Utils\SitesPage::getMediaOptionByKey($footer_options, 'footer_image'); ?>" />
                </a>
            ...
            <div class="mb-5">
                <div class="mb-2 text-base font-light text-theme-tertiary-color">COMPANY</div>
                <div class="text-base leading-7 brightness-90 text-theme-tertiary-color">
                    <?php if (!empty($news)) : ?>
                        <div>
                            <a href="/<?= $news->get('page_slug') ?>" class="brightness-[0.7] text-theme-tertiary-color no-underline"><?= $news->get('page_title'); ?></a>
                        </div>
                    <?php endif; ?>
                    <?php if (!empty($contact)) : ?>
                        <div>
                            <a href="/<?= $contact->get('page_slug') ?>" class="brightness-[0.7] text-theme-tertiary-color no-underline"><?= $contact->get('page_title'); ?></a>
                        </div>
                    <?php endif; ?>
                    <?php if (!empty($about)) : ?>
                        <div>
                            <a href="/<?= $about->get('page_slug') ?>" class="brightness-[0.7] text-theme-tertiary-color no-underline"><?= $about->get('page_title'); ?></a>
                        </div>
                    <?php endif; ?>
                </div>
            </div>
            <div class="mb-5">
                <div class="mb-2 text-base font-light text-theme-tertiary-color">PROPERTIES</div>
                <div class="text-base leading-7 brightness-90 text-theme-tertiary-color">
                    <?php if (!empty($list_a_property)) : ?>
                        <div>
                            <a href="/<?= $list_a_property->get('page_slug') ?>" class="brightness-[0.7] text-theme-tertiary-color no-underline"><?= $list_a_property->get('page_title'); ?></a>
                        </div>
                    <?php endif; ?>
                    <?php if (!empty($properties)) : ?>
                        <div>
                            <a href="/<?= $properties->get('page_slug') ?>" class="brightness-[0.7] text-theme-tertiary-color no-underline"><?= $properties->get('page_title'); ?></a>
                        </div>
                    <?php endif; ?>
                    <?php if (!empty($wishlist)) : ?>
                        <div>
                            <a href="/<?= $wishlist->get('page_slug') ?>" class="brightness-[0.7] text-theme-tertiary-color no-underline"><?= $wishlist->get('page_title'); ?></a>
                        </div>
                    <?php endif; ?>
                </div>
            </div>
            <div class="mb-5">
                <div class="mb-2 text-base font-light text-theme-tertiary-color">INFORMATION</div>
                <div class="text-base leading-7 brightness-90 text-theme-tertiary-color">
                    <?php if (!empty($discover)) : ?>
                        <div>
                            <a href="/<?= $discover->get('page_slug') ?>" class="brightness-[0.7] text-theme-tertiary-color no-underline"><?= $discover->get('page_title'); ?></a>
                        </div>
                    <?php endif; ?>
                    <?php if (!empty($privacy)) : ?>
                        <div>
                            <a href="/<?= $privacy->get('page_slug') ?>" class="brightness-[0.7] text-theme-tertiary-color no-underline"><?= $privacy->get('page_title'); ?></a>
                        </div>
                    <?php endif; ?>
                    <?php if (!empty($cookies) && \App\Utils\IntegrationsGdpr::isEnabled($site_data_options)) : ?>
                        <div>
                            <a href="/<?= $cookies->get('page_slug') ?>" class="brightness-[0.7] text-theme-tertiary-color no-underline"><?= $cookies->get('page_title'); ?></a>
                        </div>
                    <?php endif; ?>
                </div>
            </div>
            <div>
                <form class="newsletter" action="" integration-recaptcha=true data-action="newsletter">
                    <div class="sm:px-0 max-w-[300px] xl:w-[300px] mx-auto">
                        <div class="mb-2 text-base font-light brightness-90">
                            <?= $footer_options['newsletter_title'] ?>
                        </div>
                        <div class="text-base leading-7 mb-5 brightness-[0.7] text-theme-tertiary-color">
                            <?= $footer_options['newsletter_description'] ?>
                        </div>
                        <div class='relative subscribe-section w-full'>
                            <input type="text" placeholder="<?= $footer_options['newsletter_email_label'] ?>" name="email" class="field-text field-sm bg-white focus:bg-white focus:text-black w-full h-[45px]">
                        </div>
                        <div class="pb-6">
                            <button type="submit" class="btn btn-primary btn-md mt-6 w-full subscribe-me flex items-center justify-center gap-1">
