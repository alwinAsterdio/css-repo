...
<?= $this->Html->css('/mansion/css/tw-property'); ?>
<?php include __DIR__ . '/components/top-menu.php' ?>

<div class="pt-[155px] pb-[60px] relative property-item bg-[#f0efef]" id="<?= $property->get('id') ?>" data-sale-rent="<?= $property->getCustomSaleRent() ?>">
    <div class="flex flex-col lg:flex-row gap-5 lg:gap-10 lg:justify-center lg:items-center h-full px-10 max-w-[1600px] mx-auto">
        <div class="flex-1">
            <div class="inline-flex gap-1 text-sm font-semibold text-theme-primary-color border-0 border-b border-solid border-gray-300 pb-1">
                <div>#<?= $property->getField('ref'); ?></div>
                <div>-</div>
                <div><?= $property->getField('sale_rent') ?></div>
            </div>
            <div class="mt-3 property-features-section">
                <?= $this->element('property_features/short-layout-1', ['label_prefix' => ':']); ?>
            </div>
            <div class="mt-3 text-black font-[Jaldi] text-2xl"><?= $property->get('name') ?></div>
            <div class="mt-3 text-lg font-medium text-[#160301]"><?= $address ?></div>
            <div class="mt-5 text-3xl font-semibold"><?= $property->getWebsitePrice() ?></div>
...
<div class="px-5 md:px-10 pt-16 max-w-[1600px] mx-auto">
    <div class="grid grid-cols-1 lg:grid-cols-2 lg:gap-10 xl:gap-24">
        <div class="h-[250px] md:h-[450px] xl:h-[500px] mb-20">
        ...
        <div>
            <div class="text-[#333] text-3xl mb-5"><?= $page_data_options['property_desc_title'] ?></div>
    ...
    <div class="pt-10 property-features box-border w-full">
        <div class="font-[Jaldi] text-2xl font-medium"><?= $page_data_options['features_section_title']; ?></div>
        <?= $this->element('property_features/layout-1', ['no_style' => false]); ?>
    </div>
    <!-- Enquiry/Map section -->
    <div class="text-center py-10 my-10 rounded-xl border border-solid border-gray-200 bg-[#f0efef] flex px-10">
        <div class="basis-1/2">
            <div class="box-border w-full responsive-map mb-10">
                <div class="font-[Jaldi] text-2xl xl:text-3xl text-theme-primary-color-darker mb-5 lg:mb-10">
        ...
        <div class="basis-1/2">
            ...
            <div class="max-w-[700px] mx-auto box-border">
                <form id="enquiry-form" action="" class="grid w-full gap-5 enquiry-form" integration-recaptcha=true data-action="lead_form">
                    ...
                    <div class="form-group">
                        <textarea type="text" placeholder="<?= $page_data_options['enquiry_message'] ?>" id="message" name="message" rows="5" class="w-full bg-white border border-solid border-[#bbb] border-opacity-50 px-3 py-3.5 box-border rounded-md placeholder:text-[#999] required-field"></textarea>
                        <div class="form-group__helper"></div>
                    </div>
                </form>
                <div class="text-left">
