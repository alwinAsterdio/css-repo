...
?>
<a href='<?= $property->getUrl() ?>' class="text-black no-underline property-item" id="<?= $property->getField('id'); ?>" data-sale-rent="<?= $property->getCustomSaleRent() ?>">
    <div class="shadow-md min-h-[50px] aspect-ratio-[3/4] box-border">
        <div class="relative">
            <div class="relative overflow-hidden bg-gray-400">
                <div class="aspect-video"></div>
                <img class="absolute top-0 left-0 object-cover w-full h-full transition-opacity duration-500 opacity-100 cursor-pointer" src="<?= $property->getFeaturedPhotoURL('medium') ?>" alt="property-image" loading="lazy" onerror="onError.call(this)"/>
            </div>
            <span class="absolute cursor-pointer shadow-lg text-[#1d1e20] text-base py-1.5 px-3 border-transparent bg-white top-3 left-3">
                <?= $sale_rent ?>
            </span>
        ...
        <div class="box-border property-details">
            <div class="p-4 text-left details">
                ...
                <div class="text-lg font-medium name"><?= $property->get('name') ?></div>
            ...
            <div class="properties-features-section bg-[#F4F5F9] box-border text-center p-2 w-full">
                <?= $this->element('property_features/short-layout-1', ['property' => $property, 'default' => '--']); ?>
            </div>
        </div>
    </div>
</a>