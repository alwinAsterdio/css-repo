...
<?= $this->Html->css('/Neaimoveis/css/tw-propertieslisting'); ?>
...
<div class="properties-listing-page">
    <div class="bg-[var(--accent-color)]">
        <div class="px-6">
            <div class="container xl:max-w-[1140px] mx-auto titleColor">
                <div class="search-menu pt-24 pb-5 md:pb-0 border-0 border-b border-solid border-b-gray-300">
                    <?= \App\Utils\Search::render('advance', ['row_class' => 'grid-cols-1 md:grid-cols-3 lg:grid-cols-6', 'mobile_filter' => true]); ?>
                </div>
            </div>
        </div>
    </div>
            ...
            <div class="properties-listing-section">
                <?php if (!empty($property_data['data'])) : ?>
                    <div class="properties-title-container">
                        <div>
                            <h1 class="m-0 text-2xl md:text-3xl leading-tight font-bold titleColor">
                                <?= $page_data_options['properties_title'] ?>
                            </h1>
                        </div>
                        <div class="qcf-container inline-labels flex flex-wrap m-mobile:flex-row md:items-center gap-5">
                            ...
                            ?>
                            <div class="w-[180px] order-1 md:order-2">
                                <?= \App\Utils\CustomFields::fieldSelect($field_details); ?>
                            </div>
                            <div class="lg:flex-1 order-3 md:order-1 text-sm lg:text-base">
                                <?= $property_data['pagination']['paginationInfo'] ?>
                            </div>
                            <div
                                class="flex-1 md:flex-none order-2 md:order-3 inline-flex justify-end gap-2 items-center hover:cursor-pointer view-map-btn">
                                <p
                                    class="whitespace-nowrap text-theme-primary-color inline-block border border-solid border-theme-primary-color px-5 py-2.5 rounded-full hover:bg-theme-primary-color hover:text-white transition-colors">
                                    <span class="hidden sm:inline-block">VIEW ON&nbsp;</span>MAP
                                </p>
                            </div>
                        </div>
                    </div>
                ...
                <?php else : ?>
                    <div class="w-full text-center titleColor text-3xl opacity-50">No Properties</div>
                <?php endif; ?>
            </div>
            <?php if (!empty($property_data['pagination']['count'])) : ?>
                <div class="flex justify-center w-full bg-white mt-10 qcf-search-pagination">
...
<?= $this->html->script('/neaimoveis/js/propertieslisting.js'); ?>
